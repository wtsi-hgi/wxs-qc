...
submit Python tasks to worker nodes.
Therefore, they are sensitive to Python version and
environment configuration on worker nodes.

If this or following step fails with some weird messages
...
and ensure that you're using the same versions across all cluster nodes.

If the issue remains, you can run these steps on a local Spark environment:
...
def get_rf_runs(rf_json_fp: str) -> Dict:
    ...
    """
    if os.path.exists(rf_json_fp):
        print(f"Loading existing model info from {rf_json_fp}")
        with open(rf_json_fp) as f:
            return json.load(f)
    else:
