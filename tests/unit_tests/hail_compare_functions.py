...
def compare_structs(struct1, struct2):
    ...
    def check_field_equal(field):
        ...
        if struct1[field].dtype == hl.dtype("float"):
            ...
            element_wise_expr = hl.map(
                lambda a, b: (hl.is_defined(a) & hl.is_defined(b) & hl.approx_equal(a, b))
                | (~hl.is_defined(a) & ~hl.is_defined(b))
                | (hl.is_nan(a) & hl.is_nan(b)),
                struct1[field],
                struct2[field],
            )
...
def compare_plinks(
    bed1_path: str, bim1_path: str, fam1_path: str, bed2_path: str, bim2_path: str, fam2_path: str
) -> bool:
    """
    ...
    # Load the plinks
    mt1 = hl.import_plink(
        bed=path_spark(bed1_path), bim=path_spark(bim1_path), fam=path_spark(fam1_path), reference_genome="GRCh38"
    )
    mt2 = hl.import_plink(
        bed=path_spark(bed2_path), bim=path_spark(bim2_path), fam=path_spark(fam2_path), reference_genome="GRCh38"
    )
