...
def prune_pc_relate(
    ...
    # Running PCA
    union_pca_scores, pca_scores, pca_loadings, _ = run_pc_project(
    ...
    relatedness_ht = hl.pc_relate(
        pruned_mt.GT, scores_expr=union_pca_scores[pruned_mt.col_key].scores, **pc_relate_params["pc_relate_args"]
    )
    relatedness_ht = relatedness_ht.checkpoint(hl.utils.new_temp_file("pc_relate_relatedness", "ht"), overwrite=True)
    # prune individuals to be left with unrelated - creates a table containing one column - samples to remove
    pairs = relatedness_ht.filter(
        relatedness_ht[pc_relate_params["relatedness_column"]] > pc_relate_params["relatedness_threshold"]
    )
