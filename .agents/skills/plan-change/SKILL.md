...
3. Read the corresponding parts of the code and analyse existing behavior.
4. Assume required tools, permissions, credentials, data access, cloud access, Spark/Hail setup, and local configuration are already available.
   If inspection reveals an environment blocker, stop and report it instead of planning a workaround.
5. If requested behavior conflicts with the actual codebase, stop and ask for clarification instead of planning around the conflict.
6. Compose a minimal one-PR plan using the section structure from `references/plan-template.md`.
7. Save the complete proposed plan to `artifacts/1_plan.md`.
8. Keep scope tight:
   - Explicitly list out-of-scope items.
   - Avoid broad refactors if not requested explicitly.
9. Mention pipeline-contract impact explicitly:
